...
public class FootballManager {
    ...
    private static final String refereeKeyPrefix = "referee";
    private static final int numOfReferees = 100000;
    private static final int printEvery = 10000;
    public void addReferees() {
        for(int i = 0; i < numOfReferees; i++) {
            cache.put(refereeKeyPrefix + i, refereeKeyPrefix + i);
            if (i % printEvery == 0) {
                con.printf("Records inserted %1$d \n", i);
            }
        }
    }
    ...
    public void printReferees() {
        ...
        for(int i = 0; i < numOfReferees; i++) {
            ...
            //con.printf(cache.get(refereeKeyPrefix + i).toString());
            String obj = cache.get(refereeKeyPrefix + i).toString();
            if (obj == null) {
                con.printf("--->> Key not found: %1$d \n", i);
            }
            ...
            if (i % printEvery == 0) {
                con.printf("Records fetched %1$d \n", i);
            }
            //con.printf("\n");
        }
        con.printf("Fetch min=%1$d max=%2$d \n", min, max);
    }
